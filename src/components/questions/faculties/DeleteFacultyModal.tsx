...
import { useDeleteFacultyMutation, type FacultyItem } from "@/store/apis";
...
export function DeleteFacultyModal({ open, faculty, onClose }: Props) {
  ...
  const handleDelete = async () => {
    try {
      await deleteFaculty(faculty._id).unwrap();
      toast.success("Faculty deleted successfully");
      onClose();
    } catch (err: any) {
      toast.error(err?.data?.message || "Failed to delete faculty");
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="mb-1 text-lg font-bold text-[#1e293b]">Delete Faculty?</h2>
          <p className="mb-6 text-sm text-[#475569]">
            Are you sure you want to delete <span className="font-semibold text-[#0f172a]">{faculty.name}</span>? This action cannot be undone.
          </p>

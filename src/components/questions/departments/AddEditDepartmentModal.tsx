...
import { useAddDepartmentMutation, useUpdateDepartmentMutation, type DepartmentItem, type FacultyItem } from "@/store/apis";
...
export function AddEditDepartmentModal({ open, departmentToEdit, faculties, onClose }: Props) {
  if (!open) return null;

  return (
    <DepartmentModalDialog
      key={departmentToEdit?._id ?? "new"}
...
function DepartmentModalDialog({
  departmentToEdit,
  faculties,
  onClose,
}: {
  ...
  const [nameInEnglish, setNameInEnglish] = useState(departmentToEdit?.nameInEnglish || "");
  const [nameInAlbanian, setNameInAlbanian] = useState(departmentToEdit?.nameInAlbanian || "");
  const [facultyId, setFacultyId] = useState(departmentToEdit?.faculty?._id || "");

  const [errors, setErrors] = useState<{ name?: string; faculty?: string }>({});

  const validate = () => {
    const newErrors: { name?: string; faculty?: string } = {};
    if (!name.trim()) newErrors.name = "Department name is required";
    if (!facultyId) newErrors.faculty = "Parent faculty is required";
    setErrors(newErrors);
    ...
    try {
      if (isEditing && departmentToEdit) {
        await updateDepartment({
          departmentId: departmentToEdit._id,
          name: name.trim(),
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-[#334155]">
                  Parent Faculty <span className="text-rose-500">*</span>
                </label>
                <select
                  disabled={isEditing} // Cannot change parent faculty once created based on current API structure
                  value={facultyId}
                  onChange={(e) => {
                    setFacultyId(e.target.value);
                    if (errors.faculty) setErrors({ ...errors, faculty: undefined });
                  }}
                  className={`h-9 w-full rounded-lg border px-3 text-xs text-[#0f172a] outline-none transition-colors focus:border-[#2563eb] disabled:opacity-60 disabled:bg-slate-50 ${
                    errors.faculty ? "border-rose-300 bg-rose-50" : "border-[#cbd5e1] bg-white"
                  }`}
                >
                ...
                {errors.faculty && (
                  <p className="mt-1 text-[10px] text-rose-500">{errors.faculty}</p>
                )}
                {isEditing && (
                  <p className="mt-1 text-[10px] text-slate-500">Parent faculty cannot be changed after creation.</p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="h-9 w-full rounded-lg border border-[#cbd5e1] bg-white px-3 text-xs text-[#0f172a] outline-none transition-colors focus:border-[#2563eb]"
                  placeholder="e.g. Shkenca Kompjuterike"
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-[#eef2f6] px-5 py-4 bg-[#f8fafc] rounded-b-2xl">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="inline-flex min-w-[120px] items-center justify-center gap-1.5 rounded-lg bg-[#2563eb] px-4 py-2 text-xs font-semibold text-white shadow-xs transition-colors hover:bg-[#1d4ed8] disabled:opacity-50"
            >
              {isSubmitting ? (
